...
- The build failed because the external project path didn't exist. Updated `InternetSpeedTest.csproj` to default to the local `EmailerUtility` and only reference the external path when `UseExternalEmailer=true` and the csproj exists.
- Build now succeeds.
...
> Windows preserves the original creation timestamp when overwriting files on publish.
